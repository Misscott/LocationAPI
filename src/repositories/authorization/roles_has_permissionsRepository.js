...
const _rolesHasPermissionsQuery = (_pagination = '') => ({count}) => ({uuid, role_uuid, permission_uuid, roleName}) => {
    ...
    const roleUuidCondition = role_uuid ? 'AND fk_role = (SELECT id from dbmaster.roles WHERE uuid = :role_uuid)' : '';
    const roleNameCondition = roleName ? 'AND fk_role = (SELECT id from dbmaster.roles WHERE name = :roleName)' : '';
    const permissionsUuidCondition = permission_uuid ? 'AND fk_permission = (SELECT id from dbmaster.permissions WHERE uuid = :permission_uuid)' : '';
    return `
      SELECT
        ${count || 
            `r.*, 
...
const insertRolesHasPermissionsQuery = () => {
    ...
    VALUES (
      :uuid,
      (SELECT id FROM dbmaster.roles WHERE uuid = :role_uuid),
      (SELECT id FROM dbmaster.permissions WHERE uuid = :permission_uuid),
      :now,
      :createdBy
    );
    SELECT permissions.*,
